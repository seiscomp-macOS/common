...
namespace
{
  ...
  struct buffer_resource : boost::container::pmr::memory_resource
  {
    ...
    bool
    do_is_equal(const boost::container::pmr::memory_resource& other) const noexcept override
    { return &other == this; }
  ...
  using buffered_string = boost::container::pmr::string;
#else
	   ...
	   chars_format fmt) noexcept
{
...
#if _GLIBCXX_USE_CXX11_ABI
  buffer_resource mr;
  boost::container::pmr::string buf(&mr);
#else
  string buf;
  ...
  __try
    {
      if (const char* pat = pattern(first, last, fmt, buf))
